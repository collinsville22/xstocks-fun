...
from memory_optimizer import memory_manager, create_cache_cleanup_strategy
# PERFORMANCE FIX: malloc_trim for aggressive memory cleanup (fixes pandas/yfinance leak)
...
# ============================================================================
# PERFORMANCE FIX: Initialize malloc_trim for aggressive memory cleanup
# ============================================================================
# Load libc for malloc_trim (Linux only - fixes glibc memory fragmentation)
try:
    cdll.LoadLibrary("libc.so.6")
    libc = CDLL("libc.so.6")
    MALLOC_TRIM_AVAILABLE = True
    logger.info("✅ malloc_trim() loaded - aggressive memory cleanup enabled")
except Exception as e:
    MALLOC_TRIM_AVAILABLE = False
    libc = None
    logger.warning(f"⚠️ malloc_trim() not available: {e} - memory cleanup will be less aggressive")

# ============================================================================
# PERFORMANCE FIX: Helper Functions for Crash Prevention
# ============================================================================

def aggressive_memory_cleanup():
    """
    Aggressive memory cleanup using gc + malloc_trim
    Fixes pandas/yfinance memory leak on Linux
    """
    # Force garbage collection 3 times
    collected = 0
    for _ in range(3):
        collected += gc.collect()

    # Call malloc_trim to force glibc to return memory to OS
    if MALLOC_TRIM_AVAILABLE and libc:
        libc.malloc_trim(0)

    logger.debug(f"🧹 Memory cleanup: {collected} objects freed + malloc_trim called")

def check_memory_circuit_breaker():
    """
    Circuit breaker: Reject requests if memory > 85%
    Prevents OOM kills on t3.micro (1GB RAM)
    """
    memory_percent = psutil.virtual_memory().percent
    if memory_percent > 85:
        logger.error(f"🚨 CIRCUIT BREAKER TRIGGERED: Memory at {memory_percent}% - rejecting request")
        raise HTTPException(
            status_code=503,
            detail=f"Service temporarily overloaded (memory: {memory_percent}%). Please try again in 30 seconds."
        )
    return memory_percent

# ============================================================================
# PERFORMANCE FIX: Request Semaphore (Concurrency Limiter)
# ============================================================================
# Max 2 concurrent heavy requests to prevent memory spikes
...
class MemoryMonitoringMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request, call_next):
        ...
        if memory_manager.check_memory_pressure():
            logger.warning("High memory usage detected before request")
            memory_manager.aggressive_cleanup()

            # If still high memory after cleanup, clear old caches
            if memory_manager.check_memory_pressure():
                cleanup_strategy = create_cache_cleanup_strategy(
                    memory_manager,
                    _stock_data_cache
                )
                cleanup_strategy()
        ...
        # Periodically log memory stats (5% of requests)
...
# MEMORY OPTIMIZED: Increased to 15 min to align with frontend polling + async cache
_stock_data_cache = {
    ...
    'cache_duration': 900  # 15 minute cache (increased from 5 min to reduce refetch load)
}

# Cache for indices data (NO CHART DATA) - 1 hour cache
_indices_data_cache = {}  # Period-specific cache dictionary

# Cache for index charts (reduced to 5 min) - key format: "{symbol}_{period}"
_index_chart_cache = {'cache_duration': 300}
    ...
    try:
        ...
        def fetch_single_stock(symbol):
            ...
            try:
                ...
                # MEMORY OPTIMIZED: Fetch minimal history needed for technical indicators (RSI needs 14+ days, MACD needs 26+ days)
                # Reduced from up to 2y to max 6mo to prevent OOM on t3.micro (1GB RAM)
                if period in ["1D", "1d"]:
                    hist_period = "2mo"  # Reduced from 3mo
                elif period in ["1W", "1w"]:
                    hist_period = "2mo"  # Reduced from 3mo
                elif period in ["1M", "1m"]:
                    hist_period = "3mo"  # Reduced from 6mo
                elif period in ["3M", "3m"]:
                    hist_period = "4mo"  # Reduced from 1y
                elif period in ["YTD", "ytd"]:
                    hist_period = "6mo"  # Reduced from 1y
                elif period in ["1Y", "1y"]:
                    hist_period = "6mo"  # Reduced from 2y - still enough for comparison
                else:
                    hist_period = "2mo"  # Default to 2 months (enough for indicators)
                ...
                if not hist.empty and len(hist) > 0:
                    ...
                    # PERFORMANCE FIX: Skip options for /all-xstocks to save 150MB+ memory
                    # Options data still available via dedicated /api/options/* endpoints
                    options_volume = 0
                    put_call_ratio = 0
                    options_open_interest = 0
                    # Commented out to reduce memory usage and API calls:
                    # try:
...
async def get_batch_realtime_data_v2(symbols: str = Query(..., description="Comma-separated xStock symbols like AAPLx,TSLAx")):
    """
    BATCH ENDPOINT - Get real-time data for multiple xStock symbols (LIGHTWEIGHT)
    Returns array of real-time data for all requested symbols
    PERFORMANCE: Uses only ticker.info (80-120MB) vs ticker.history + earnings (700-900MB)
    """
    # PERFORMANCE FIX 1: Circuit breaker - reject if memory > 85%
    memory_percent = check_memory_circuit_breaker()

    # PERFORMANCE FIX 2: Request semaphore - max 2 concurrent requests
    async with REQUEST_SEMAPHORE:
        ...
        logger.info(f"📊 Batch request for {len(xstock_symbols)} symbols (memory: {memory_percent:.1f}%)")
        ...
        async def fetch_one_symbol(xstock_symbol: str):
            ...
            try:
                ...
                return {
                    "symbol": xstock_symbol,
                    "name": info.get('shortName', xstock_symbol),
        ...
        # PERFORMANCE FIX 3: Aggressive memory cleanup after heavy operation
...
async def get_all_xstocks():
    ...
    # Add async caching with 15-minute TTL
    cache_key = "all_xstocks_1d"
    cached = await get_cache(cache_key)
    if cached:
        logger.info("💾 Returning cached all-xstocks data")
        return cached

    logger.info("🔄 Fetching fresh all-xstocks data")
    ...
    result = clean_data_for_json(intel_data)

    # Cache for 15 minutes (900 seconds)
    await set_cache(cache_key, result, ttl_seconds=900)
    logger.info("✅ Cached all-xstocks data for 15 minutes")
